...
let tHead = ["Nombre", "Descripción", "Acciones"];
    ...
    return (
      <td style={columnButtonStyle}>
        ...
        {permisos.filter((permiso) => permiso.id == 19).length > 0 ? (
          <button
            style={buttonStyle}
            onClick={this.eliminarCategoria}
            type="button"
...
class ListadoCategorias extends Component {
  ...
  render() {
    ...
    if (categorias.length === 0) {
      ...
      return (
        <SortableTbl
          tblData={categorias.sort(function (a, b) {
            return b.id - a.id;
          })}
          tHead={tHead}
          Permisos={this.props.usuario}
          customTd={[{ custd: ActionCategoriasComponent, keyItem: "Actions" }]}
          dKey={col}
          search={true}
          defaultCSS={true}
