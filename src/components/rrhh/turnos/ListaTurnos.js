...
let tHead = ["Turno", "Hora de Entrada", "Hora de Salida", "Acciones"];
...
class ListaTurnos extends Component {
  ...
  render() {
    ...
    if (turnos.length === 0) {
      ...
      return (
        <SortableTbl
          tblData={turnos.sort(function (a, b) {
            return b.id - a.id;
          })}
          tHead={tHead}
          Permisos={this.props.usuario}
          customTd={[{ custd: ActionTurnoComponent, keyItem: "Actions" }]}
          dKey={col}
          search={true}
          defaultCSS={true}
