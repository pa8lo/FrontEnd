...
let tHead = ["Gasto", "Monto", "Fecha", "Acciones"];
...
class ListaGastos extends Component {
  ...
  render() {
    ...
    if (gastos.length === 0) {
      ...
      return (
        <SortableTbl
          tblData={gastos.sort(function (a, b) {
            return b.id - a.id;
          })}
          tHead={tHead}
          Permisos={this.props.usuario}
          customTd={[{ custd: ActionGastoComponent, keyItem: "Actions" }]}
          dKey={col}
          search={true}
          defaultCSS={true}
