...
class SolicitudesEncoladas extends Component {
...

    {
      this.state.pedidos.map(
        (datos) => (
          ...
          (this.state.datos_cliente = this.state.clientes.filter(
            (client) => client.id === datos.client
          )),
          /** Direcciones */
          ...
          this.state.datos_cliente.map((direccion_pedido) =>
            this.state.pedidos.map(
              (direccion_storeada) =>
                (this.state.dire_cliente = direccion_pedido.Adress.filter(
                  (dire) => dire.id === direccion_storeada.address
                ))
            )
          ),
          /** Combos */
          ...
          datos.combo.length > 0
            ? datos.combo.map(
                (combos_storeados) =>
                  (this.state.combos_cliente = this.state.combos.filter(
                    (combo) => combo.id === combos_storeados.Offer
                  ))
              )
          ...
          datos.product.length > 0
            ? datos.product.map(
                (pedidos_storeados) =>
                  (this.state.producto_cliente = this.state.productos.filter(
                    (product) => product.id === pedidos_storeados.Product
                  ))
              )
            : (this.state.combos_cliente = [])
  ...
  mostrarDatos = () => {
    ...
    if (this.state.existen_datos) {
      return this.state.pedidos.map((datos) => (
