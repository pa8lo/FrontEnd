...
export const mostrarCategoria = (id) => async (dispatch) => {
  const categoria = await axios.get(
    `https://roraso.herokuapp.com/Category/Category?id=${id}`,
    { headers: { "access-token": localStorage.getItem("access-token") } }
  );
...
export const agregarCategoria = (categoria) => async (dispatch) => {
  ...
  await axios
    .post("https://roraso.herokuapp.com/Category/CreateCategory", data, {
      headers: { "access-token": localStorage.getItem("access-token") },
    })
    .then((res) => {
      if (res.status === 200) {
        ...
        Swal.fire({
          title: "Error!",
          text: "Se ha producido un error al intentar crear la categoría",
          type: "error",
          confirmButtonText: "Aceptar",
        });
        return;
    ...
    .catch((err) => {
      ...
      if (localStorage.getItem("enviarCat").length > 0) {
        ...
        localStorage.setItem(
          "enviarCat",
          JSON.stringify(deserializarCategoriaAEnviar)
        );
      } else {
...
export const eliminarCategoria = (id) => async (dispatch) => {
  await axios
    .post(
      "https://roraso.herokuapp.com/Category/DeleteCategory",
      { id: id },
      { headers: { "access-token": localStorage.getItem("access-token") } }
    )
    .then((res) => {
      if (res.status === 200) {
        Swal.fire({
          title: "Correcto!",
          text: "Se ha borrado una categoría",
          type: "success",
          confirmButtonText: "Confirmar",
        });
        setTimeout(function () {
        ...
        Swal.fire({
          title: "Error!",
          text: "Se ha producido un error al intentar borrar la categoría",
          type: "error",
          confirmButtonText: "Aceptar",
        });
        return;
...
export const editarCategoria = (categoria) => async (dispatch) => {
  ...
  await axios
    .post("https://roraso.herokuapp.com/Category/UpdateCategory", data, {
      headers: { "access-token": localStorage.getItem("access-token") },
    })
    .then((res) => {
      if (res.status === 200) {
        ...
        Swal.fire({
          title: "Correcto!",
          text: "Se ha actualizado una categoría",
          type: "success",
          confirmButtonText: "Confirmar",
        });
        setTimeout(function () {
        ...
        Swal.fire({
          title: "Error!",
          text: "Se ha producido un error al intentar actualizar la categoría",
          type: "error",
          confirmButtonText: "Aceptar",
        });
        return;
